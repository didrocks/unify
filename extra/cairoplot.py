...
class BarPlot(Plot):
    def __init__(self, 
        ...
        self.bounds[HORZ] = h_bounds
        self.bounds[VERT] = v_bounds
        self.force_max_value = max_value
    ...
    def calc_boundaries(self):
        ...
        if not self.bounds[VERT]:
            max_data_value = min_data_value = 0
            if self.force_max_value:
                 max_data_value = self.force_max_value
            for series in self.data:
                if max(series) > max_data_value and not self.max_value:
                    max_data_value = max(series)
                if min(series) < min_data_value:
                    min_data_value = min(series)
            self.bounds[VERT] = (min_data_value, max_data_value)
...
def bar_plot(name, 
    ...
    plot = BarPlot(name, data, width, height, background, border,
                   grid, rounded_corners, three_dimension, h_labels, v_labels, h_bounds, v_bounds, colors, max_value)
    plot.render()
    plot.commit()
